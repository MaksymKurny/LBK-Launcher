...
const StoreButton: React.FC<StoreLinkProps> = ({ type = 'steam', appId, url }) => {
  ...
  return (
    <a
      data-nav-group="main-links"
      className={`group flex items-center gap-2 px-4 py-2 rounded-lg bg-glass hover:bg-glass-hover border border-border hover:border-[${config.color}]/60 hover:shadow-lg hover:shadow-[${config.color}]/20 transform hover:scale-[1.02] transition-all duration-200 ease-out`}
      title={`Відкрити в ${config.title}`}
      href={config.url}
      target="_blank"
    >
      <div
        className={`text-[${config.color}] group-hover:brightness-125 transition-all duration-200 ease-out`}
      >
        {config.icon}
      </div>
